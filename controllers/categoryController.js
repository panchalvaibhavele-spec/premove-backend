...
export const scheduleVisit = (req, res) => {
  ...
  db.query(
    ...
    (err, rows) => {
      ...
      db.query(
        "SELECT cust_name, cust_mobile, city_name AS customer_city FROM ele_customer_lead WHERE id = ? LIMIT 1",
        [lead_id],
        (err2, leadRows) => {
          if (err2 || !leadRows.length) {
            return res
              .status(404)
              .json({ success: false, message: "Lead not found" });
          }
          ...
          // STEP 3: Find city manager
          db.query(
            `SELECT id, name, phone_number, assign_location 
             FROM ele_customer_manager 
             WHERE TRIM(LOWER(assign_location)) = TRIM(LOWER(?)) 
               AND status='active'
             LIMIT 1`,
            [leadInfo.customer_city],
            async (err3, managerRows) => {
              let manager = managerRows[0];

              // IF NO CITY MANAGER FOUND → USE HEAD MANAGER
              if (!manager) {
                manager = await new Promise((resolve) => {
                  db.query(
                    `SELECT id, name, phone_number 
                     FROM ele_customer_manager 
                     WHERE user_role='Head_Manager' LIMIT 1`,
                    (err4, headRows) => {
                      if (err4 || !headRows.length) resolve(null);
                      else resolve(headRows[0]);
                    }
                  );
                });
              }
              ...
              if (!manager) {
                return res.json({
                  success: false,
                  message: "No available manager found",
                });
              }

              // FUNCTION TO INSERT & SEND WHATSAPP (SAME LIKE YOUR VIDEO API)
              const insertVisit = () => {
                const insertQuery = `
                  INSERT INTO customer_visite_request 
                  (lead_id, customer_name, customer_number, customer_city, schedule_date, time_from, time_to, manager_id, status)
                ...
                db.query(
                  ...
                  async (err4, result) => {
                    ...
                    const message = `📅 *New Home Visit Request*\n
👤 *Customer:* ${leadInfo.cust_name}
📱 *Mobile:* ${leadInfo.cust_mobile}
📍 *City:* ${leadInfo.customer_city}
📆 *Date:* ${schedule_date}
⏰ *Time:* ${time_from} - ${time_to}

If you want to *reschedule*, please open the app and choose *Reschedule Visit*.`;

                    try {
                      await fetch(
                        `${WHATSAPP_API_URL}?token=${WHATSAPP_TOKEN}&phone=91${
                    ...
                    db.query(
                      ...
                      (err5, insertedRows) => {
                        return res.json({
                          success: true,
                          message: "Visit scheduled & manager notified",
...
export const customerAcceptReschedule = (req, res) => {
  ...
  db.query(q, [request_id], (err) => {
    ...
    // 2️⃣ Fetch updated full visit request + customer details
    const fetchQuery = `
      SELECT 
        r.*,
        c.cust_name,
        c.cust_mobile,
        c.city_name
      FROM customer_visite_request r 
      JOIN ele_customer_lead c ON c.id = r.lead_id
      WHERE r.id=?
    `;

    db.query(fetchQuery, [request_id], async (err2, rows) => {
      if (err2 || !rows.length) {
        return res.json({
          success: true,
          message: "Accepted, but failed to fetch details",
        });
      }
      ...
      // 3️⃣ Fetch Manager Phone Number
      db.query(
        `SELECT phone_number, name FROM ele_customer_manager WHERE id=? LIMIT 1`,
        [manager_id],
        async (err3, mgrRows) => {
          if (err3 || !mgrRows.length) {
            return res.json({
              success: true,
              message: "Accepted, but manager not found",
            });
          }

          const managerPhone = mgrRows[0].phone_number;
          const managerName = mgrRows[0].name;

          // 4️⃣ WhatsApp Message to Manager
          const message = `✅ *Customer Accepted Reschedule*

👤 *Customer:* ${cust_name}
📱 *Mobile:* ${cust_mobile}
📍 *City:* ${city_name}

📅 *New Visit Date:* ${manager_reschedule_date}
⏰ *New Time:* ${manager_reschedule_time_from} - ${manager_reschedule_time_to}

The customer has accepted the rescheduled visit.`;
          ...
          return res.json({
            success: true,
            message: "Reschedule accepted & manager notified",
          });
        }
      );
    });
...
export const getLeadCount = (req, res) => {
  const sql = `
  ...
  db.query(sql, (err, result) => {
    if (err) {
      console.log("Lead Count Error:", err);
      return res.status(500).json({ success: false });
    }

    console.log("Lead Count Result:", result);

    return res.json({
      success: true,
      count: result[0].total_leads,
    });
  });
};

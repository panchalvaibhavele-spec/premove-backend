...
export const createVisitRequest = async (req, res) => {
  ...
  try {
    ...
    // ❌ Only COMPLETED visit cannot start
    if (visit.status === "completed") {
      return res.json({
        success: false,
        error: "Inspection already completed for this customer",
    ...
    // 🟢 Allow visit if status is:
    // pending, approved, started, reschedule_by_manager, reschedule_by_customer
    await db
      .promise()
      .query(
        "UPDATE customer_visite_request SET manager_id = ? WHERE lead_id = ?",
    ...
    return res.json({
      success: true,
      message: "Visit request allowed. You can start inspection.",
    });
  } catch (err) {
    console.error("Create visit request error:", err);
    return res.json({ success: false, error: "Server error" });
